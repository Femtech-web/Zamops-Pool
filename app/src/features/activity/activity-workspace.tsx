...
function ActivityRowAmount({ item }: { item: ActivityItem }) {
  const { t } = useI18n();
  ...
  if (!item.encryptedAmountHandle) return <span className="activity-row-no-amount" aria-label={t("activity.amountPrivate")}>—</span>;
  return <div className="activity-row-amount">
    <strong>{amount === undefined ? "••••••" : `${formatUnits(amount, 6)} ${item.assetSymbol ?? ""}`}</strong>
    <button type="button" onClick={() => amount === undefined ? void reveal() : setAmount(undefined)} disabled={decrypting} aria-label={decrypting ? t("activity.revealing") : amount === undefined ? t("activity.revealAmount") : t("activity.hideAmount")}>
      {amount === undefined ? <Eye size={13} /> : <EyeOff size={13} />}
    </button>
  </div>;
}
...
function ActivityDetails({ item }: { item: ActivityItem }) {
  ...
  const close = () => router.replace("/app/activity");
  useDialogA11y(dialogRef, true, close);
  ...
  return <div className="activity-detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
    <section ref={dialogRef} className="activity-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="activity-detail-title" aria-describedby="activity-detail-description">
      ...
      <p className="activity-detail-copy" id="activity-detail-description">{item.detail}</p>
      <dl>
