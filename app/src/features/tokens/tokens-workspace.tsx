...
export function TokensWorkspace() {
  ...
  return <main className="pool-shell" id="main-content" tabIndex={-1}>
    <AppHeader />
    {!isConnected || !address ? <TokensWalletGate /> : <TokensPortfolio key={address} assets={assets} account={address} isLoading={isLoading} error={error} />}
    <OperationFeedback />
  </main>;
}
...
function TokensPortfolio({ assets, account, isLoading, error }: { assets: PoolAsset[]; account: Address; isLoading: boolean; error: Error | null }) {
  ...
  const [batchReveal, setBatchReveal] = useState<{ id: number; balances: Map<string, bigint> }>({ id: 0, balances: new Map() });
  const [revealingAll, setRevealingAll] = useState(false);
  const balanceReads = useReadContracts({ contracts: assets.map((asset) => ({ address: asset.confidentialTokenAddress, abi: confidentialWrapperAbi, functionName: "confidentialBalanceOf" as const, args: [account] as const })) });
  const balanceHandles = useMemo(() => assets.flatMap((asset, index) => {
    const read = balanceReads.data?.[index];
    return read?.status === "success" ? [{ address: asset.confidentialTokenAddress, handle: read.result as Hex }] : [];
  }), [assets, balanceReads.data]);

  async function revealAll() {
    if (!balanceHandles.length || balanceHandles.length !== assets.length || revealingAll) return;
    setRevealingAll(true);
    try {
      activity.begin({ title: t("operation.revealTitle"), detail: t("operation.revealSign"), step: 1, totalSteps: 2 });
      await grantPermit.mutateAsync(balanceHandles.map((item) => item.address));
      activity.progress(t("operation.revealDecrypt"), 2);
      const result = await sdk.decryption.decryptValues(balanceHandles.map((item) => ({ encryptedValue: item.handle, contractAddress: item.address })));
      const next = new Map<string, bigint>();
      for (const item of balanceHandles) {
        const value = result[item.handle];
        if (value === undefined) throw new Error("Private values unavailable");
        next.set(item.address.toLowerCase(), BigInt(value));
      }
      setBatchReveal((current) => ({ id: current.id + 1, balances: next }));
      activity.fail();
      activity.notifySuccess(t("tokens.allBalancesRevealed"), t("tokens.allBalancesRevealedBody"));
    } catch (cause) {
      activity.fail();
      activity.notifyError(toUserFacingError(cause, t));
    } finally { setRevealingAll(false); }
  }

  if (isLoading) return <WorkspaceSkeleton label={t("tokens.loading")} variant="tokens" />;
  ...
  return <section className="tokens-page" aria-labelledby="tokens-title">
    <header className="tokens-heading"><div><p className="eyebrow">{t("tokens.kicker")}</p><h1 id="tokens-title">{t("tokens.title")}</h1><p>{t("tokens.description")}</p></div>{assets.length ? <button type="button" onClick={() => batchReveal.balances.size ? setBatchReveal((current) => ({ id: current.id + 1, balances: new Map() })) : void revealAll()} disabled={revealingAll || balanceReads.isLoading || balanceHandles.length !== assets.length}>{batchReveal.balances.size ? <EyeOff size={13} /> : <Eye size={13} />}{batchReveal.balances.size ? t("tokens.hideAll") : t("tokens.revealAll")}</button> : null}</header>
    <div className="tokens-list">{assets.map((asset) => <TokenRow key={`${asset.confidentialTokenAddress}:${batchReveal.id}`} asset={asset} account={account} batchBalance={batchReveal.balances.get(asset.confidentialTokenAddress.toLowerCase())} batchRevealId={batchReveal.id} />)}</div>
  </section>;
}
...
function TokenRow({ asset, account, batchBalance, batchRevealId }: { asset: PoolAsset; account: Address; batchBalance?: bigint; batchRevealId: number }) {
  ...
  const [rowBalance, setRowBalance] = useState<bigint>();
  const [hiddenBatchId, setHiddenBatchId] = useState<number>();
  ...
  const privateBalance = rowBalance ?? (hiddenBatchId === batchRevealId ? undefined : batchBalance);
  useDialogA11y(actionDialogRef, action !== null, closeAction);
  ...
  function hidePrivateBalance() {
    setRowBalance(undefined);
    setHiddenBatchId(batchRevealId);
  }
  ...
  async function reveal() {
    await run(async () => {
      ...
      if (value === undefined) throw new Error("Private values unavailable");
      setRowBalance(BigInt(value));
      setHiddenBatchId(undefined);
      activity.fail();
